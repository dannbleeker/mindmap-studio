...
// obvious place to add new branches (e.g. the slash-command menu). The DOM side effects (execCommand,
// reading innerHTML) stay in the caller's action callbacks; this function only decides + dispatches.
...
export interface EditorKeyActions {
  /** Apply an inline format via the contentEditable (Ctrl/Cmd + B/I/U). */
...
export function handleEditorKeyDown(e: EditorKeyEvent, actions: EditorKeyActions): boolean {
  ...
  if ((e.ctrlKey || e.metaKey) && /^[biu]$/i.test(e.key)) {
    ...
    actions.format(k === "b" ? "bold" : k === "i" ? "italic" : "underline");
    return true;
  }
  if (e.key === "Enter") {
    e.preventDefault();
