...
- [ ] Phase 2 — node images rendering on the canvas, and capturing canvas-drawn
      boundaries back into the model (boundaries currently render one-way from import).
      (Presentation mode, self-contained HTML export, print-to-PDF, node search,
      multi-file batch import, boundaries rendering, lazy-load, and relationship arrows
      are done.)
