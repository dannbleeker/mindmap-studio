...
// Permanent clearance test for the node's lower-right corner: the drag-to-relate grip must never overlap
// the (always-visible) collapse toggle on a right-growing node with children. The px helper mirrors the
// CSS `top` exactly (shared constants), so this guards the real layout, not a parallel copy.
...
describe("relate-grip vs collapse-toggle clearance", () => {
  ...
  it("the grip clears the right-side collapse toggle at every node height", () => {
    for (const h of HEIGHTS) {
      const grip = relateGripRectPx(h, /* rightToggle */ true);
  ...
  it("without a right-side toggle the grip keeps its full below-centre offset", () => {
    for (const h of HEIGHTS) {
      const grip = relateGripRectPx(h, /* rightToggle */ false);
      // top point = 50% + 16; element top = that − 8 (translateY(-50%)).
      expect(grip.top).toBeCloseTo(0.5 * h + GRIP_BELOW_CENTER - 8, 5);
    }
  });

  it("emits a clamped min() only when a right toggle is present", () => {
